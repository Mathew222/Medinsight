...
function Analyze() {
  ...
  const fetchAnalysis = async () => {
    ...
    try {
      ...
      console.log(`Fetching analysis for file: ${filename} (Path: ${filePath})`);

      const response = await axios.post(`${backendUrl}/analyze`, {
        file_path: filePath, // Backend needs the full path stored locally
        filename: filename   // Send filename for context if needed by backend
      }, { withCredentials: true }); // Send cookies for session context
      ...
      <main className="flex-grow overflow-y-auto p-4 md:p-8">
        ...
        {summary && !loadingAnalysis && !analysisError && (
          ...
          <div className="max-w-5xl mx-auto space-y-5 md:space-y-6">
             {/* Header Section for the Analysis */}
             <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4 p-4 bg-white rounded-lg shadow">
                 <div className="flex items-center space-x-3">
                    <Stethoscope className="h-9 w-9 text-blue-600 flex-shrink-0" />
                    <div>
                        <h1 className="text-2xl md:text-3xl font-bold text-gray-800">Medical Report Analysis</h1>
                        {filePath && <p className="text-sm text-gray-500 break-all">File: {filePath.split(/[\\/]/).pop()}</p>}
                    </div>
                 </div>
                  {/* Booking Button */}
                 <button
                   onClick={handleBooking}
                   className="px-5 py-2.5 bg-green-600 text-white text-base font-semibold rounded-lg flex items-center space-x-2 shadow-md hover:bg-green-700 transition duration-200 flex-shrink-0 w-full sm:w-auto"
                 >
